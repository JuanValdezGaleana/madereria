...
<table width="100%" border="1" class="propTabla">
  ...
  <?php while($rDat=mysql_fetch_array($cRepo)){ $id_tipo_prod=$rDat['ID_TIPO_PRODUCTO'];?>
  <tr class="hover_lista">
    ...
    <td align="center"><?php 
										$cRepo1=mysql_query("
				  SELECT SUM(B.CANTIDAD_ACTUAL) AS CANT
FROM productos A
INNER JOIN inventario B ON A.ID_PRODUCTO = B.ID_PRODUCTO
WHERE A.ID_TIPO_PRODUCTO ='$id_tipo_prod'

				   ")
or die("No se pudo hacer la consulta. </br>".mysql_error());	
	$rDat1=mysql_fetch_array($cRepo1);	   
											   echo $rDat1['CANT'];  ?></td>
