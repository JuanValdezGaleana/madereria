...
$cIDProd=mysql_query(" SELECT ID_PRODUCTO,PRECIO_FINAL FROM productos WHERE CVE_PRODUCTO='$entra_codigo' AND ID_ESTABLECIMIENTO='$id_establecimiento'")
	or die("Error al consultar el identifocador del producto. <br/>.".mysql_error());
...
$opc;	
	if($num==0){$opc=1;} /* SI NO HAY REGISTROS A $opc LE ASIGNAMOS EL NÚMERO 1 PARA QUE ENTRA AL SWITCH */
	...
	switch($opc){
		...
		/*////  SI NO SE HA ECHO LA PRIMERA VENTA, NO SE HA GENERADO LA CLAVE DE VENTA PARA HACER SU CONSECUTIVO. ENTONCES LA GENERAMOS   */	
			...
			$cVenta=mysql_query("
					...
					'','$clave_venta',1,NOW(),$id_persona,$id_producto,3,'','',$id_establecimiento,$precio_venta,0
					  )")
