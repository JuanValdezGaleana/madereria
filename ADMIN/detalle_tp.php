...
<table width="100%" border="1" class="propTabla">
  ...
  while($rDat=mysql_fetch_array($cRepo)){  $i=$i+1;
   ?>
   <form action="upd_precio_compra.php" method="post">
  ...
  if($rDat['EXISTENCIA'] > $alerta1 && $rDat['EXISTENCIA'] <= $alerta2){
    ...
    <td align="right" <?php if($rDat['PRECIO_FINAL']<$rDat['PRECIO_COMPRA']){echo 'bgcolor="#FF0000" style="color:#FFF"';} ?> ><input type="text" style="width: 80px; border: none;" name="precio_c" value="<?php echo $rDat['PRECIO_COMPRA']; ?>"></td>
    <input type="hidden" value="<?php echo $rDat['ID_PRODUCTO']; ?>" name="id_producto">
    <input type="hidden" value="<?php echo $tp; ?>" name="tp">
    <td align="right"><input type="text" style="width: 80px; border: none;" name="precio_v" value="<?php echo $rDat['PRECIO_VENTA']; ?>"></td>
    <!--<td align="right"><?php echo $rDat['DESCUENTO']." %"; ?></td>-->
    <td align="right" <?php if($rDat['PRECIO_FINAL']<$rDat['PRECIO_COMPRA']){echo 'bgcolor="#FF0000" style="color:#FFF"';} ?> ><input type="text" style="width: 80px; border: none;" name="precio_f" value="<?php echo $rDat['PRECIO_FINAL']; ?>"></td>
   ...
   <td><input type="submit" class="boton_verde" value="Guardar"></td>
    </tr>
  </form>
<?php 
      $sumaAux=$sumaAux+$sum;
